...
export function registerRoutes(app) {
    ...
    app.get("/api/tasks", async (req, res) => {
        ...
        try {
            ...
            const tasks = await storage.getTasksByUser(req.user.id, range);
            res.json(tasks);
        }
        catch (error) {
    ...
    app.patch("/api/settings/profile", async (req, res) => {
        ...
        try {
            ...
            const updates = {};
            if (name)
                updates.name = InputSanitizer.sanitizeTaskTitle(name);
            if (email)
    ...
    app.patch("/api/settings/enforcement", async (req, res) => {
        ...
        try {
            ...
            const updates = {};
            if (strictModeEnabled !== undefined)
                updates.strictModeEnabled = strictModeEnabled;
            if (uninstallProtectionEnabled !== undefined)
                updates.uninstallProtectionEnabled = uninstallProtectionEnabled;
    ...
    app.patch("/api/settings/notifications", async (req, res) => {
        ...
        try {
            ...
            if (taskReminders !== undefined)
                updates.notificationTaskReminders = taskReminders;
            if (streakUpdates !== undefined)
                updates.notificationStreakUpdates = streakUpdates;
            if (accountabilityAlerts !== undefined)
                updates.notificationAccountabilityAlerts = accountabilityAlerts;
            const user = await storage.updateUserSettings(req.user.id, updates);
            if (!user) {
                return res.status(404).json({ message: "User not found" });
            }
    ...
    app.get("/api/calendar/events", async (req, res) => {
        ...
        try {
            ...
            const events = await storage.getCalendarEvents(req.user.id, startDate, endDate);
            res.json(events);
        }
        catch (error) {
